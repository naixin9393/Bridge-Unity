    ...
    public class StaymanSecondResponseStrategy : IBiddingStrategy {
        ...
        private List<BiddingSuggestion> StaymanSpadeBranchResponses(BiddingContext context) {
            ...
            if (isFitSpades && is3SCall && totalPointsSpades >= 15 && totalPointsSpades <= 16)
                suggestions.Add(new(message: Stayman2S3SSecondResponsePass(totalPointsSpades),
                    call: new Pass(null)));

            if (isFitSpades && is3SCall && totalPointsSpades == 17)
                suggestions.Add(new(message: Stayman2S3SSecondResponse4S(totalPointsSpades),
                    call: new BidCall(new Bid(4, Strain.Spades), null)));

            // Response to 4S
            if (is4SCall)
                suggestions.Add(new(message: AuctionConcludedPass,
                    call: new Pass(null)));

            // Response to 2NT
            if (is2NTCall) {
                if (hcp >= 15 && hcp <= 16)
                    suggestions.Add(new(message: Stayman2S2NTSecondResponsePass(hcp),
                        call: new Pass(null)));
                if (hcp == 17)
                    suggestions.Add(new(message: Stayman2S2NTSecondResponse3NT(hcp),
        ...
        private IEnumerable<BiddingSuggestion> StaymanHeartBranchResponses(BiddingContext context) {
            ...
            if (isFitHearts && is3HCall && totalPointsHearts >= 15 && totalPointsHearts <= 16)
                suggestions.Add(new(message: Stayman2H3HSecondResponsePass(totalPointsHearts),
                    call: new Pass(null)));

            // 4H when 17 TP
            if (isFitHearts && is3HCall && totalPointsHearts == 17)
                suggestions.Add(new(message: Stayman2H3HSecondResponse4H(totalPointsHearts),
            ...
            if (is2NTCall) {
                if (!isFitSpades) {
                    if (hcp >= 15 && hcp <= 16)
                        suggestions.Add(new(message: Stayman2H2NTSecondResponsePass(hcp),
                            call: new Pass(null)));
                    if (hcp == 17)
                        suggestions.Add(new(message: Stayman2H2NTSecondResponse3NT(hcp),
                            call: new BidCall(new Bid(3, Strain.NoTrump), null)));
                }
                else {
                    // 3S when 15 to 16 TP
                    if (totalPointsSpades >= 15 && totalPointsSpades <= 16)
                        suggestions.Add(new(message: Stayman2H2NTSecondResponse3S(totalPointsSpades),
                            call: new BidCall(new Bid(3, Strain.Spades), null)));

                    // 4S when 17 TP
                    if (totalPointsSpades == 17)
                        suggestions.Add(new(message: Stayman2H2NTSecondResponse4S(totalPointsSpades),
                            call: new BidCall(new Bid(4, Strain.Spades), null)));
                }
            }
            ...
            if (is3NTCall) {
                ...
                if (isFitSpades && totalPointsSpades >= 15 && totalPointsSpades <= 17)
                    suggestions.Add(new(message: Stayman2H3NTSecondResponse4S(totalPointsSpades),
                        call: new BidCall(new Bid(4, Strain.Spades), null)));

                if (!isFitSpades && hcp >= 15 && hcp <= 17)
                    suggestions.Add(new(message: Stayman2H3NTSecondResponsePass(hcp),
                            call: new Pass(null)));
            }
        ...
        private IEnumerable<BiddingSuggestion> StaymanDiamondBranchResponses(BiddingContext context) {
            ...
            if (is2NTCall && hcp >= 15 && hcp <= 16)
                suggestions.Add(new(message: Stayman2D2NTSecondResponsePass(hcp),
                    call: new Pass(null)));

            if (is2NTCall && hcp == 17)
                suggestions.Add(new(message: Stayman2D2NTSecondResponse3NT(hcp),
                    call: new BidCall(new Bid(3, Strain.NoTrump), null)));
